...
export function createCloud({onSnapshot,onStatus}) {
  let api='',session=null,timer,inflight=false,stopped=false,revision=-1,lastConfig=0,configPromise=null;
  ...
  function config(){
    ...
    configPromise=(async()=>{
      ...
      try{
        const r=await fetch(`${import.meta.env.BASE_URL}cloud-config.json?t=${Math.floor(Date.now()/60000)}`,{cache:'no-store',signal:controller.signal});
        if(!r.ok)throw Error('无法读取联机配置。');
        const c=await r.json();if(c.apiUrl){const url=new URL(c.apiUrl);if(url.protocol!=='https:'&&!(url.protocol==='http:'&&['localhost','127.0.0.1'].includes(url.hostname)))throw Error('联机地址必须使用 HTTPS。');api=url.origin;}
        lastConfig=Date.now();return !!api;
      }catch(e){if(e.name==='AbortError'||e instanceof TypeError)throw Error('暂时读取不到联机地址，请稍后重试。');throw e;}
      finally{clearTimeout(timeout);}
    })().finally(()=>{configPromise=null;});
    return configPromise;
  }
  const definiteRejection=e=>e.status>=400&&e.status<500&&![408,429].includes(e.status);
  ...
  async function poll(){if(!session||inflight||stopped)return;inflight=true;try{if(!api)await config();accept(await request('snapshot'));}catch(e){onStatus('offline',session);if(e.status===401)onStatus('invalid',session);if(api&&Date.now()-lastConfig>60000)try{await config();}catch{}}finally{inflight=false;schedule();}}
  ...
  async function attach(path,{name,invite,token}){const previous=session;const next={token:token||secret(),invite:path==='create'?(invite||secret()):null};
    ...
    session=next;save();
    try{const data=await request(path,{name,invite:invite||next.invite},next.token);revision=-1;accept(data);schedule();return data;}catch(e){if(definiteRejection(e)){session=previous;save();}throw e;}}
  ...
  return {
    ...
    async start(){if(session?.cached){validate(session.cached);onSnapshot({state:session.cached,slot:session.slot,roomId:session.roomId,memberCount:session.memberCount,cached:true});}try{await config();if(session)await poll();else onStatus(api?'available':'unconfigured');}catch(e){onStatus('offline',session);schedule();throw e;}},
    ...
    async action(action){if(!session?.roomId)throw Error('先创建或加入一个房间。');const pending=session.pending;
      ...
      try{const result=await request('action',body);clearPending(body.requestId);accept(result);schedule();return {...result,requestId:body.requestId};}catch(e){if(definiteRejection(e))clearPending(body.requestId);throw e;}},
    async retry(){const pending=session?.pending;try{await config();}catch(e){if(!api)throw e;}if(pending){if(session?.pending?.requestId===pending.requestId)return this.action(pending.action);return;}await poll();},
