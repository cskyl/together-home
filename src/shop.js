import './shop.css';
import { referencePhoto,productDetails } from './product-details.js';
import { furnitureThumbnail } from './furniture-thumbnails.js';
import { items,categories,furnitureGroups,getItem,quote,optionGroups,itemName,configName,placementRooms,placementSlots } from './items.js';
...
export function createShop({getState,mutate,getPerson,getError,showInRoom,toast}){
  ...
  document.querySelectorAll('[data-furniture-group]').forEach(b=>b.onclick=()=>{roomGroup=b.dataset.furnitureGroup;expanded=false;render(true);});
  $('#shop-sort').onchange=e=>{sort=e.target.value;render(true);};
  $('#shop-budget').oninput=e=>{budget=e.target.value;expanded=true;render(true);};
  ...
  function render(force=false){
    const state=getState(),all=inventory(state),key=JSON.stringify([state.selected,balance(state),all,state.placements,state.customPlans,ownedView,category,expanded,search,roomGroup,sort,budget]);
    ...
    $('#shop-grid').querySelectorAll('[data-reference-photo]').forEach(img=>img.onerror=()=>{const item=getItem(img.dataset.referencePhoto);img.outerHTML=thumbnail({...item,reference:null});});
