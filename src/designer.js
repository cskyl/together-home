...
import { saveDesign,inventory } from './state.js';
...
export function createDesigner({getState,mutate,getError,getScope,onSaved,onDecorate}){
  ...
  function render(){
    $('#design-name').value=draft.name;$('#design-summary').textContent=`${draft.rooms.length} / 20 个房间 · ${draft.rooms.reduce((n,r)=>n+r.w*r.d/4,0)} m²`;
  ...
  document.querySelectorAll('[data-design-template]').forEach(b=>b.onclick=()=>{checkpoint();const next=designTemplate(b.dataset.designTemplate);draft={...next,id:draft.id,name:baseVersion?draft.name:next.name};selected=draft.rooms[0]?.id;selectedOpening=null;changed('模板已套用。房间、门窗、颜色都可以接着改。');});
  ...
  async function save(){
    if(busy)return;try{validateDesign(draft);}catch(e){setMessage(e.message);return;}
