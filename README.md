...
现在也可以在商店买东西，摆进 3D 房间里：21 套 LEGO 主街景系列、6 种原创车型、4 个盲盒系列（共 24 款）、6 款娃娃、10 件装饰摆件、38 件日常家具和 10 件猫猫家具。LEGO 名称与参考图来自官方，房间里的模型为程序生成的简化展示；家具与车辆增加官方商品图片、规格和来源，房间里的 3D 仍是同类物品的简化示意；盲盒和娃娃为原创设计。商店购买价全部是游戏资金，官方参考零售价在详情里单独显示。
...
在选房区域点 **自己设计户型**，可以从空白或三套起步模板开始，包括带独立猫房的小屋。当前支持单层：24 × 24 米画布、每格 0.5 米、每套最多 20 个矩形房间，可以拼出不同外形；同一双人房间最多保存 8 套自定义户型。
...
自定义户型不对应真实房源或市场报价，可在设计器填写房子总预算（默认 $450,000 游戏资金，范围 $1,000–$100,000,000）。预算不能低于已经投入的施工金额；改布局不改变预算，修改预算会按新总额重算完成比例，历史金额不变。`src/designs.js` 包含共享的几何和校验规则，`src/designer.js` 是编辑界面；`npm run test:designer` 和 `npm run test:designer-online` 验证绘制、拖动、装修、草稿恢复、触摸操作、并发冲突和重试。默认联机测试使用隔离内存后端，`DESIGN_LIVE_API=1` 使用公开后端的独立测试房间。

## 施工预算与收支账本

每套官方户型的完整施工预算等于页面显示的官方起价，例如 Riverside 为 $456,900。20 个步骤按固定游戏比例分配费用，整数舍入差额计入相邻步骤，所有步骤合计与总预算完全一致。施工按顺序推进，每次只投入当前步骤剩余费用以内的可用余额，完工后不能超额施工。阶段比例用于游戏进程，不是实际工程报价。

原来的学习、购买和施工记录不重算，不补扣资金；已投入的金额继续计入该户型，新完成比例按新的房子总预算计算。施工包含房屋基础配置；在商店额外买的收藏品、车、家具等是独立消费，不计入施工预算。

从钱包或最近记录进入 **收支账本**，可以查看学习收入、施工支出、商店支出和逐笔累计余额，按类型、成员和日期筛选，也可翻页查看早期记录。新施工记录保存具体步骤和操作成员；旧施工记录未保存成员的，显示为共同施工。所有金额取自已保存的账目，联机双方共享同一份记录。

`npm run test:construction` 检查预算合计、旧资金保留、自定义预算校验、账本筛选和分页，以及隔离后端的双人施工记账；运行前先启动开发前端。`npm test` 还会检查六套房价格、20 步金额舍入、最终金额封顶、成员权限和请求去重。

## 商店和摆放

「家具」现在有 38 件，按客厅、卧室、书房、餐厅、厨房、卫浴、玄关、户外分组。新增转角沙发、沙发床、双层床、梳妆台、升降桌、文件柜、厨房中岛、收纳推车、洗手台柜、鞋柜、衣帽架和户外桌椅等 26 件；「猫猫家具」有猫爬架、猫窝、猫床、猫砂盆、双碗喂食台、饮水机、猫抓柱、隧道、窗边休息架和玩具篮，共 10 件。每件都有独立的 3D 模型，可以旋转查看，购买后摆放。游戏中购买的是简化模型，价格是游戏资金。家具、猫用品和装饰的原有游戏价已降到约三成（按 5 取整，最低 15），例如双人床 1,000 → 300、猫爬架 650 → 195；新家具为 20–220 游戏资金。

想做猫房：打开 **自己设计户型 → 带猫房小屋**，或手动添加用途为 **猫房** 的房间；保存后，到 **商店 → 猫猫家具** 选购。买完选择那套户型和猫房，在俯视图点按、拖动或用滑块安排位置，旋转朝向后保存。猫房默认是空房，方便自己装；其他房间也可以关掉默认家具再布置。摆放与旋转免费，购买作为商店支出单独记账，双方都能接着摆。

1. 点施工面板下的 **逛商店 / 摆放物品**，或者顶部的 **商店**。
2. 可按房间、游戏预算和价格排序筛选，也能搜索品牌或型号。商品卡片显示官方实物参考图；详情有尺寸、材质或车辆性能等参数、参考零售价、核对日期和官网链接，3D 仍可旋转查看。车辆有车漆、轮毂、内饰、车顶和外观套件；LEGO 可加防尘罩和暖光。
...
商品清单在 `src/items.js`、`src/furniture-catalog.js`，实物资料在 `src/furniture-references.js`、`src/car-references.js` 和 `src/decor-references.js`；官方图片及资料来源记录在 `research/furniture-sources.json`、`research/car-sources.json`、`research/decor-sources.json` 和原有 `research/shop-sources.json`。图片保存在站点本地，正常浏览不依赖外站热链。资料核对日期为 2026-09-20，实物参考价不是实时报价，也不决定游戏扣款；实物型号、规格、颜色与简化 3D 可能不同。车辆游戏选配不是原厂配置表。

`src/prices.js` 固定保存原 69 件商品的第 1 版基础价。缺少 `priceVersion` 的历史购买按第 1 版验证，仍使用原成交金额；新购买保存 `priceVersion: 2` 并采用较低游戏价。没有回溯退款或重算余额，也不重置施工、仓库及摆放。后端决定成交价和版本，客户端传入的金额/版本不能改变结算；请求去重仍有效。未来再次改价需要保留第 2 版价格并增加版本，不能改写历史价格。`tests/prices.test.js` 检查历史金额、旧新购买共存、禁止伪造金额、SQLite 重启和重试。

真实商品验证：`npm run test:products` 检查本地图像解码、按房间/预算/价格筛选、品牌搜索、规格和参考价展示、旧新购买共存、手机布局及双人摆放；部署验收可设置 `PRODUCT_LIVE_API=1` 和 `TEST_URL=https://cskyl.github.io/together-home/`，只使用独立测试房间。

猫房验证：`npm run test:cats` 检查带猫房模板、猫家具购买与摆放、双人同步、刷新保留和手机布局；默认使用隔离内存后端。部署验收可同时设置 `PET_LIVE_API=1` 和 `TEST_URL=https://cskyl.github.io/together-home/`，只创建独立测试房间。
...
- 官方户型起价、查询日期与原网页；按价格、面积排序，按起价预算筛选。页面显示的官方起价同时作为该户型的游戏施工总预算，价格仍按查询日期手动维护。
- 每次可记录 1–480 分钟；以每分钟 $10 游戏资金为基准，按投入状态和小幅随机效率结算实际收入。
- 共同资金，每个户型独立保存施工进度。20 个施工步骤从测量、清场、地基到框架、屋顶、水电、内墙、厨卫及完工，每一步的预算合计严格等于该房子的总预算。
