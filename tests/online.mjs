...
try{
  ...
  await b.goto(invite);await b.locator('#connect-name').fill('联机测试乙');await b.locator('#join-room').click();await expect(b.locator('#connected-summary')).toContainText('已经在同一个家');await b.locator('#close-cloud').click();await expect(a.locator('#together-names')).toContainText('联机测试乙',{timeout:25000});
  ...
  await c.locator('.restore-details summary').click();await c.locator('#recovery-input').fill('TH1-'+ownerRecovery);await c.locator('#restore-form button').click();await expect(c.locator('#connected-summary')).toContainText('已经在同一个家');await c.locator('#close-cloud').click();await expect(c.locator('#sync-open')).toContainText('联机测试甲');
