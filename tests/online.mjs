import { buildPercent,money,studyCredit,cloudState } from './browser-helpers.mjs';
...
try{
  ...
  await expect.poll(async()=>(await cloudState(a)).events.filter(e=>e.type==='study').length,{timeout:25000}).toBe(2);const firstTotal=studyCredit(await cloudState(a)),builtPercent=buildPercent(firstTotal);await expect(a.locator('#balance')).toHaveText(money(firstTotal),{timeout:25000});await expect(b.locator('#balance')).toHaveText(money(firstTotal),{timeout:25000});
  await Promise.all([a.locator('#invest').click(),b.locator('#invest').click()]);await expect(a.locator('#balance')).toHaveText('$0',{timeout:25000});await expect(b.locator('#balance')).toHaveText('$0',{timeout:25000});await expect(a.locator('#build-percent')).toHaveText(builtPercent);await expect(b.locator('#build-percent')).toHaveText(builtPercent);
  ...
  await a.locator('#study-open').click();await a.locator('#minutes').fill('25');await a.locator('#study-note').fill('响应丢失后安全重试');await a.locator('#study-form button[type=submit]').click();await expect(a.locator('#toast')).toContainText('未确认');await a.locator('#study-dialog .close-dialog').click();await a.locator('#sync-open').click();await a.locator('#retry-sync').click();await expect.poll(async()=>(await cloudState(a)).events.some(e=>e.note==='响应丢失后安全重试'),{timeout:25000}).toBe(true);const retryEvent=(await cloudState(a)).events.find(e=>e.note==='响应丢失后安全重试');await expect(a.locator('#balance')).toHaveText(money(retryEvent.reward),{timeout:25000});await expect(b.locator('#balance')).toHaveText(money(retryEvent.reward),{timeout:25000});await a.locator('#close-cloud').click();await expect(a.locator('.journal-entry').filter({hasText:'响应丢失后安全重试'})).toHaveCount(1);
  await b.locator('#invest').click();await expect(a.locator('#balance')).toHaveText('$0',{timeout:25000});await expect(b.locator('#build-percent')).toHaveText(buildPercent(retryEvent.reward,'anthem'));
  await a.locator('.plan-select[data-plan=riverside]').click();await expect(b.locator('#plan-title')).toHaveText('Riverside',{timeout:25000});await expect(b.locator('#build-percent')).toHaveText(builtPercent);
