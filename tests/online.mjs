import { money,studyCredit,cloudState } from './browser-helpers.mjs';
...
try{
  ...
  await expect.poll(async()=>(await cloudState(a)).events.filter(e=>e.type==='study').length,{timeout:25000}).toBe(2);const firstTotal=studyCredit(await cloudState(a)),builtPercent=Math.round(firstTotal/5500*100)+'%';await expect(a.locator('#balance')).toHaveText(money(firstTotal),{timeout:25000});await expect(b.locator('#balance')).toHaveText(money(firstTotal),{timeout:25000});
  ...
  await b.locator('#invest').click();await expect(a.locator('#balance')).toHaveText('$0',{timeout:25000});await expect(b.locator('#build-percent')).toHaveText(Math.round(retryEvent.reward/5500*100)+'%');
