...
try{
  ...
  await expect(b.locator('#balance')).toHaveText(money(credited-constructionSpent-4020));const first=(await snapshot(b)).events.find(e=>e.item==='box-forest');expect(first.variant).toMatch(/^[0-5]$/);
  await a.locator('#item-close').click();await a.locator('#sync-open').click();await a.locator('#retry-sync').click();await expect(a.locator('#balance')).toHaveText(money(credited-constructionSpent-4020));await a.locator('#close-cloud').click();
  ...
  const final=await snapshot(b);expect(final.events.filter(e=>e.type==='study')).toHaveLength(3);expect(final.events.filter(e=>e.type==='build')[0].amount).toBe(constructionSpent);expect(final.events.find(e=>e.item==='box-forest')).toEqual(first);
