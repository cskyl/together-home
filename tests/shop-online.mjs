...
try{
  ...
  await Promise.all([study(a,480),study(b,100)]);await expect.poll(async()=>(await snapshot(a)).events.filter(e=>e.type==='study').length).toBe(2);const credited=studyCredit(await snapshot(a));await expect(a.locator('#balance')).toHaveText(money(credited));await a.locator('#invest').click();await expect(b.locator('#balance')).toHaveText(money(credited-500));
  ...
  await expect(b.locator('#balance')).toHaveText(money(credited-4520));const first=(await snapshot(b)).events.find(e=>e.item==='box-forest');expect(first.variant).toMatch(/^[0-5]$/);
  await a.locator('#item-close').click();await a.locator('#sync-open').click();await a.locator('#retry-sync').click();await expect(a.locator('#balance')).toHaveText(money(credited-4520));await a.locator('#close-cloud').click();
