...
async function study(page,minutes){await page.locator('#study-open').click();await page.locator('#minutes').fill(String(minutes));await page.locator('#study-form button[type=submit]').click();await expect(page.locator('#study-dialog')).not.toBeVisible();}
try{
  ...
  await Promise.all([study(a,480),study(b,100)]);await expect(a.locator('#balance')).toHaveText('$5,800');await a.locator('#invest').click();await expect(b.locator('#balance')).toHaveText('$5,300');
  await a.locator('[data-category=cars]').click();await a.locator('[data-item=car-coupe]').click();const config={paint:'red',wheels:'bronze',cabin:'ivory',roof:'open',trim:'sport'};for(const [k,v]of Object.entries(config))await a.locator(`[data-option=${k}]`).selectOption(v);await a.locator('#buy-item').click();await a.locator('#place-item').click();await expect(a.locator('#item-dialog')).not.toBeVisible();await expect(b.locator('#balance')).toHaveText('$1,900');
  await b.locator('[data-category=cars]').click();await b.locator('#shop-inventory').click();await expect(b.locator('[data-owned]')).toHaveCount(1);await b.locator('[data-owned]').click();await expect(b.locator('#item-description')).toContainText('酒红');await b.locator('#rotate-item').click();await b.locator('#place-item').click();await expect.poll(async()=>(await snapshot(a)).placements[0].rotation).toBe(1);
  ...
  await a.locator('[data-category=lego]').click();await a.locator('[data-item=lego-10182]').click();await a.locator('[data-option=display]').selectOption('lit');await a.locator('#buy-item').click();await a.locator('#placement-room').selectOption('study');await a.locator('#place-item').click();await expect(b.locator('#balance')).toHaveText('$1,400');
  ...
  await expect(b.locator('#balance')).toHaveText('$1,280');const first=(await snapshot(b)).events.find(e=>e.item==='box-forest');expect(first.variant).toMatch(/^[0-5]$/);
  await a.locator('#item-close').click();await a.locator('#sync-open').click();await a.locator('#retry-sync').click();await expect(a.locator('#balance')).toHaveText('$1,280');await a.locator('#close-cloud').click();
  ...
  await b.reload();await expect(b.locator('#sync-open')).toContainText('已联机');await expect(b.locator('#owned-count')).toHaveText('3');await expect(b.locator('#build-percent')).toHaveText('9%');await expect(b.locator('#scene canvas')).toHaveAttribute('data-placed','3');
  const final=await snapshot(b);expect(final.events.filter(e=>e.type==='study')).toHaveLength(2);expect(final.events.filter(e=>e.type==='build')[0].amount).toBe(500);expect(final.events.find(e=>e.item==='box-forest')).toEqual(first);
