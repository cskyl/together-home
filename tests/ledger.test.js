...
test('账本按保存顺序汇总全部收入和支出，每笔余额保留上下文而不是筛选后重算',()=>{
  ...
  const before=structuredClone(state),entries=ledgerEntries(state);assert.deepEqual(entries.map(e=>e.event.id),state.events.map(e=>e.id));assert.deepEqual(entries.map(e=>e.amount),[1000,500,200,100,600]);assert.deepEqual(entries.map(e=>e.running),[1000,500,700,600,0]);assert.deepEqual(entries.map(e=>e.incoming),[true,false,true,false,false]);assert.equal(entries.at(-1).running,balance(state));
  const purchases=entries.filter(e=>e.event.type==='purchase');assert.equal(purchases[0].running,600);assert.deepEqual(state,before);assert.equal(ledgerEntries(freshState()).length,0);
});
