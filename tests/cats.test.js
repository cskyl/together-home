...
const furnitureShapes=['sofa','armchair','bed','wardrobe','dresser','nightstand','desk','officechair','diningtable','diningchair','tvstand','bookcase','sectional','ottoman','coffeetable','sidetable','chaise','daybed','bunkbed','vanity','standingdesk','filingcabinet','bookshelfwide','stool','barstool','sideboard','kitchenisland','kitchencart','wallcabinet','bathvanity','bathshelf','mirror','laundrybasket','shoecabinet','coatstand','entrybench','outdoorchair','outdoortable'];
...
test('38件家具和10件猫用品可购买，旧47件历史价格及选配保持兼容',()=>{
  assert.equal(items.length,95);assert.equal(new Set(items.map(item=>item.id)).size,95);assert.deepEqual(items.filter(i=>i.category==='furniture').map(i=>i.id).sort(),furnitureShapes.map(s=>'furniture-'+s).sort());assert.deepEqual(items.filter(i=>i.category==='cats').map(i=>i.id).sort(),catShapes.map(s=>'cat-'+s).sort());
...
test('48件家具和猫用品使用各自的有效立体结构，模型落地且能独立创建和释放',()=>{
...
test('双人购买猫用品由后端计价并去重，重启后保留猫房、购买和共同摆放',()=>{
  ...
  try{const a=secret(),b=secret(),invite=secret();store.create(a,{name:'甲',invite});store.join(b,{name:'乙',invite});act(store,a,{type:'study',minutes:480});const design=designTemplate('cat-home');act(store,a,{type:'design',design,expectedVersion:0});const cat=design.rooms.find(room=>room.type==='cat'),requestId=randomUUID(),action={type:'purchase',item:'cat-tree',person:1,amount:0,config:{}};
    const first=act(store,a,action,requestId),owned=first.state.events.at(-1);assert.equal(owned.person,0);assert.equal(owned.amount,195);assert.equal(owned.priceVersion,2);act(store,a,action,requestId);assert.equal(inventory(store.snapshot(b).state).length,1);assert.throws(()=>act(store,b,action,requestId),/编号已被使用/);
    act(store,b,{type:'place',id:owned.id,position:{plan:design.id,room:cat.id,u:22,v:76,rotation:3}});act(store,b,{type:'purchase',item:'cat-fountain'});const before=store.snapshot(a);assert.equal(balance(before.state),4550);assert.equal(ledgerEntries(before.state).filter(entry=>!entry.incoming).length,2);store.close();store=createStore(path,{studyRoll:()=>500});assert.deepEqual(store.snapshot(b).state,before.state);assert.equal(act(store,a,action,requestId).revision,before.revision);validate(store.snapshot(a).state);
