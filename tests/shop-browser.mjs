...
try{
  ...
  await page.addInitScript(()=>{if(!localStorage.getItem('together-home-v1'))localStorage.setItem('together-home-v1',JSON.stringify({version:1,selected:'riverside',names:['我','你'],events:[...Array.from({length:6},(_,i)=>({id:'old-study-'+i,type:'study',person:0,minutes:480,note:'旧记录',at:'2026-09-01T12:00:00.000Z'})),{id:'old-build',type:'build',plan:'riverside',amount:500,at:'2026-09-01T12:01:00.000Z'}]}));});await page.goto(url,{waitUntil:'domcontentloaded'});await expect(page.locator('.shop-card')).toHaveCount(8);await page.locator('#shop-more').click();await expect(page.locator('.shop-card')).toHaveCount(21);
  ...
  await open(car.id);
  for(const [key,value] of Object.entries(config))await page.locator(`[data-option=${key}]`).selectOption(value);
  await expect(page.locator('.checkout-total strong')).toHaveText(money(carCost));await expect(page.locator('#item-preview canvas')).toHaveAttribute('data-config',JSON.stringify(config));await expect(page.locator('.vehicle-price-breakdown')).toContainText(car.optionGroups.trim.values[1].name);
  await page.locator('#item-dialog').screenshot({path:'test-results/shop-car-config.png'});
  await page.locator('#buy-item').click();await expect(page.locator('#place-item')).toBeVisible();await place('garage');await expect(page.locator('#balance')).toHaveText(money(credited-carCost-500));await expect(page.locator('#scene canvas')).toHaveAttribute('data-placed','1');
  ...
  await open('lego-10182');await page.locator('[data-option=display]').selectOption('lit');await page.locator('#buy-item').click();await place('study',0);await expect(page.locator('#balance')).toHaveText(money(credited-carCost-1000));
  ...
  for(const item of items.filter(i=>!i.archived)){await open(item.id);await expect(page.locator('#item-preview canvas')).toBeVisible();await page.locator('#item-close').click();}
  console.log(`PASS: all ${items.length} product models open and close`);
  await page.setViewportSize({width:390,height:844});await open(items.find(i=>i.optionGroups&&i.reference.brand==='Porsche').id);await expect(page.locator('#buy-item')).toBeDisabled();
