...
try{
  ...
  await page.locator('#study-open').click();await page.locator('#minutes').fill('480');await page.locator('#study-form button[type=submit]').click();await page.locator('#invest').click();await expect(page.locator('#balance')).toHaveText('$4,300');
  ...
  await page.locator('#buy-item').click();await expect(page.locator('#place-item')).toBeVisible();await place('garage');await expect(page.locator('#balance')).toHaveText('$900');await expect(page.locator('#scene canvas')).toHaveAttribute('data-placed','1');
  ...
  await open('lego-10182');await page.locator('[data-option=display]').selectOption('lit');await page.locator('#buy-item').click();await place('study',0);await expect(page.locator('#balance')).toHaveText('$400');
  await open('box-forest');await expect(page.locator('.variant')).toHaveCount(6);await expect(page.locator('.odds-note')).toContainText('没有隐藏款');await page.locator('#buy-item').click();await expect(page.locator('#item-eyebrow')).toContainText('开到了');const variant=(await data()).events.at(-1).variant;await page.locator('#rotate-item').click();await place('study',1);await expect(page.locator('#balance')).toHaveText('$280');
  await open('plush-bear');await page.locator('#buy-item').click();await place('study',12);await expect(page.locator('#balance')).toHaveText('$100');
  await open('decor-plant');await page.locator('#buy-item').click();await place('study',13);await expect(page.locator('#balance')).toHaveText('$0');await expect(page.locator('#scene canvas')).toHaveAttribute('data-placed','5');await expect(page.locator('#build-percent')).toHaveText('9%');
