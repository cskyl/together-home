...
const store=createStore(),server=createApi(store,{rateLimit:false});
...
const errors=[];
for(const page of [local,a,b]){page.setDefaultTimeout(20000);page.on('pageerror',error=>errors.push(error.message));}
for(const page of [a,b])await page.route('**/cloud-config.json*',route=>route.fulfill({contentType:'application/json',body:JSON.stringify({apiUrl:`http://127.0.0.1:${server.address().port}`})}));
...
try{
  ...
  await local.goto(url);await expect(local.locator('#balance')).toHaveText('$500');await expect(local.locator('#build-percent')).toHaveText('9%');expect(await localState(local)).toEqual(initial);
  ...
  await local.reload();expect(await localState(local)).toEqual(saved);await expect(local.locator('#balance')).toHaveText(money(500+reward.reward));await local.locator('#undo').click();await expect(local.locator('#balance')).toHaveText('$500');expect(await localState(local)).toEqual(initial);await expect(local.locator('#build-percent')).toHaveText('9%');
  console.log('PASS: keyboard slider, live range, recorded credit, reload, actual-amount undo and unmodified legacy progress');

  await a.goto(url);await a.locator('#connect-name').fill('投入测试甲');await a.locator('#create-room').click();await expect(a.locator('#cloud-connected')).toBeVisible();await a.locator('#make-invite').click();await expect(a.locator('#invite-output')).toBeVisible();const invite=await a.locator('#invite-link').inputValue();await a.locator('#close-cloud').click();
  await b.goto(invite);await b.locator('#connect-name').fill('投入测试乙');await b.locator('#join-room').click();await expect(b.locator('#connected-summary')).toContainText('两个人都加入了');await b.locator('#close-cloud').click();
  ...
  await b.reload();await expect(b.locator('#sync-open')).toContainText('已联机');expect(await cloudState(b)).toEqual(paired);await b.locator('#undo').click();await expect(a.locator('#balance')).toHaveText(money(low.reward));await expect(b.locator('#balance')).toHaveText(money(low.reward));
  ...
  console.log('REWARDS BROWSER PASSED (local saves + isolated paired API): '+url);
